...
### 실시간 환율 변환
```shell
cvtr convert <금액> <소스 통화> to <타겟 통화> [--rate|-R <환율 값>]
```
...
### 역사적 화폐 가치 계산
```shell
cvtr history <금액> <통화> <시작 연도> to <종료 연도>
```

과거 특정 연도의 화폐가 현재 어느 정도의 구매력을 갖는지, 혹은 현재의 화폐가 과거에는 어느 정도의 가치였는지 계산합니다. `<시작 연도>` 또는 `<종료 연도>`에 `current`를 사용하여 현재 시점을 나타낼 수 있습니다. 사용 가능한 연도는 **1965-2024**로 제한됩니다.
